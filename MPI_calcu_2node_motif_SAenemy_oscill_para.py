...
###########produce the motif that has two interaction between protein A and B###########
######################## and at least has a negative feedback ##########################
motif = []  #use to save motif structure
       
m=2         #couple_matrix.shape (m, n)
n=2

element = [-1.0, 0.0, 1.0]   #interaction  -1.0:inhibit,  0.0:no interaction  1.0:activation

x = [np.reshape(np.array(i), (m, n), order = 'C') for i in product(element, repeat = m*n)]
  
for j in range(len(element)**(m*n)):
    ...
    for k_x in range(m):        
        ...
        for k_y in range(n):            
            ...
            couple_matrix.append(a[k_x][k_y])
            
    if(abs(couple_matrix[1])==1.0 and abs(couple_matrix[2])==1.0): #only save the motif that has two interaction between protein A and B
        
        if(couple_matrix[0]==-1.0 or couple_matrix[1]+couple_matrix[2] ==0.0  or couple_matrix[3]==-1.0): #delete the motif has no negative feedback            
            
            motif.append(couple_matrix)  
...
for i in range(data[0], data[1]): 
    ...
    couple_matrix = motif[i, :]        
    
    filename = 'Motif' + str(int(motif[i,0]))+str(int(motif[i,1]))+str(int(motif[i,2]))+str(int(motif[i,3])) + 'oscill_para.csv'
    ...
    np.savetxt('LHS'+str(i)+'.csv', LHS_of_paras, delimiter = ',') #保存参数  
